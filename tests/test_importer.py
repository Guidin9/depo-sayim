...
# ------------------------------------- Tiger .xls uzantisiyla xlsx yaziyor
def test_xls_uzantili_xlsx_okunur(c, tmp_path):
    """Regresyon (2026-08-27, gerçek Tiger dosyasıyla üretildi).

    Ankara deposunun 24.08.2026 raporunun adı `... ankara depo.xls`, içeriği
    ise OOXML (ZIP). openpyxl `load_workbook(yol)` çağrısında dosyanın
    İÇERİĞİNE değil ADINA bakıp `InvalidFileException` atıyordu; o istisna
    `YuklemeHatasi` olmadığı için router da yakalamıyordu — kullanıcı Tiger'dan
    indirdiği dosyayı yüklerken sayımın İLK adımında HTTP 500 ve yığın izi
    alıyordu.

    Uzantıyı elle düzeltmek çözüm değil: dosyayı Tiger üretiyor, her rapor
    indirişinde aynı şey olacak.
    """
    from tests.conftest import VERI_DOSYA
    sahte_xls = tmp_path / "tigerdan gelen rapor.xls"
    sahte_xls.write_bytes(open(VERI_DOSYA, "rb").read())     # içerik xlsx, adı .xls

    ozet = importer.yukle(c, str(sahte_xls))
    assert ozet["satir"] > 0, ".xls adlı xlsx okunamadı"
    assert ozet["kaynak"] == "seri_lot"


def test_gercek_eski_xls_anlasilir_hata_verir(c, tmp_path):
    """Gerçek OLE2 .xls okunamaz — ama yığın izi değil, ne yapacağını söyle."""
    eski = tmp_path / "eski.xls"
    eski.write_bytes(bytes.fromhex("d0cf11e0a1b11ae1") + bytes(512))
    with pytest.raises(importer.YuklemeHatasi) as h:
        importer.yukle(c, str(eski))
    assert "xlsx" in str(h.value).lower(), "kullanıcıya ne yapacağı söylenmeli"


def test_excel_olmayan_dosya_anlasilir_hata_verir(c, tmp_path):
    sahte = tmp_path / "rapor.xlsx"
    sahte.write_bytes(b"bu bir excel dosyasi degil")
    with pytest.raises(importer.YuklemeHatasi):
        importer.yukle(c, str(sahte))
